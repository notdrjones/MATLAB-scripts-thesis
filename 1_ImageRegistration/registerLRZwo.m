function [fixed, movingRegistered] = registerLRZwo(stackpath, outputfolder, tform)
...
for i=1:Nframes
    movingRegistered(:,:,i) = imwarp(moving(:,:,i),tform,"OutputView",imref2d(size(fixed)));
end
...
saveastiff(uint16(movingRegistered), [outputfolder 'R.tiff'],options);
end
