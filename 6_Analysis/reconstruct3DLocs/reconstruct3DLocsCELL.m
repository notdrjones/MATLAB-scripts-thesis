function [locCell3D] = reconstruct3DLocsCELL(datafolder,saveflag)
    ...
    if ~isempty(currentLocs)
        for j=1:size(currentLocs,1)
            ...
            if ~isempty(locCellR{i})
                [xR, yR, matchingFlag] = autoMatchPoint([xL yL], locCellR{i},30);
            end
            ...
            if ~isnan(matchingFlag)
                ...
                % Rotate x,y
                [xRotated, yRotated] = rotatePoints(x,y, deg2rad(theta(i)),L/2,L/2); % theta +90
                
                currentLocs3D(end+1,:) = [i xRotated yRotated z./0.12];
            end
        end
...
if saveflag
    ...
    saveas(gcf,[datafolder 'localisations3D.fig']);
    save([datafolder 'localisations3D.mat'],'locCell3D');
    close(f);
end
