function wrapper_plotkymographs_agar(folderPath, options)
...
%     k = options.cellIDlist(i);
    currBac = bacTracks{k,:};
    cell_len = ceil(currBac(1,4));
    widthcheck = currBac(1,5) > 20;
    % widthcheck = zeros(currBac(1,1));
    check1 = any(currBac(1,1) < (cell_len/2 + options.distfromedge));
    check2 = any(currBac(1,2) < (cell_len/2 + options.distfromedge));
    check3 = any(currBac(1,1) > (options.cropField(3)-cell_len/2 - options.distfromedge));
    check4 = any(currBac(1,2) > (options.cropField(4)-cell_len/2 - options.distfromedge));
    checkarray = [check1, check2, check3, check4];
    % disp(checkarray);
    if any(checkarray) || widthcheck ==1
        disp('excluding cell');
    else
    % f = plotkymograph(currBac, fluorescenceStack, k, options);
        f = plotkymographStaticCell(currBac, fluorescenceStack, k, options);
