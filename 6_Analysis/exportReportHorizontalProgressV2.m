function exportReportHorizontalProgressV2(datafolder,skipfolder)
...
skipfolder = cell2mat(skipfolder);
if nargin>1
    folderlist = folderlist(~skipfolder);
end

plotTypes = {'horizontalProgressFigure';
    'horizontalProgressDifferenceFigure';
    'horizontalVelocityDifferenceFigure'};

for jj=1:length(plotTypes)
    ...
    for i=1:nFolders
        ...
        try
        % Open figure
        figurePath = [datafolder folderlist{i} 'translatedAndRotated\' plotTypes{jj} '.fig'];
        ...
        if mod(i,9)==0 || i == nFolders
            ...
            set(f,'Color','w');
            export_fig(f,[datafolder 'report' plotTypes{jj} '.pdf'],'-append','-nofontswap','-nocrop');
            clf
            tiledLay = tiledlayout(3,3);
            tiledLay.TileSpacing = 'loose';
            tiledLay.Padding = 'loose';
