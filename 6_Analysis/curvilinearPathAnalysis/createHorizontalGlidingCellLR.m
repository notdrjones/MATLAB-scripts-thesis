function [outputArg1,outputArg2] = createHorizontalGlidingCellLR(datafolder,saveflag)
...
fluorescenceStackL = loadtiff([datafolder 'L_cropped.tif']);
fluorescenceStackR = loadtiff([datafolder 'R_cropped.tif']);

meanFluoL = mean(fluorescenceStackL(:));
meanFluoR = mean(fluorescenceStackR(:));
...
fluorescenceRotatedAndTranslatedL = brightfieldRotatedAndTranslated-mean(brightfieldStack(:))+meanFluoL;
fluorescenceRotatedAndTranslatedR = brightfieldRotatedAndTranslated-mean(brightfieldStack(:))+meanFluoR;
...
for i=1:size(brightfieldStack,3)
    ...
    fluorescenceStackL(:,:,i) = imrotateFill(fluorescenceStackL(:,:,i),-theta(i),meanFluoL);
    fluorescenceStackR(:,:,i) = imrotateFill(fluorescenceStackR(:,:,i),-theta(i),meanFluoR);
...
try
    ...
    fluorescenceRotatedAndTranslatedL(1:101,50+(1:101)+ceil(xRotatedAndTranslated(i)),i) = fluorescenceStackL(:,:,i);
    fluorescenceRotatedAndTranslatedR(1:101,50+(1:101)+ceil(xRotatedAndTranslated(i)),i) = fluorescenceStackR(:,:,i);
catch
    a =1;
end
...
if saveflag
    ...
    saveastiff(fluorescenceRotatedAndTranslatedL,[savefolder 'fluorescenceRotatedAndTranslatedL.tif']);
    saveastiff(fluorescenceRotatedAndTranslatedR,[savefolder 'fluorescenceRotatedAndTranslatedR.tif']);
