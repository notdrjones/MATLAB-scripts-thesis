function [allTracks] = trackBacteria2D(bacLoc, stackFolder, options)
...
V = cellfun(@(x) calc_v(x(:,2),x(:,3),options.pxsize,options.dt), allTracks);
filter_index = V>=options.minSLV;
...
allTracks = cellfun(@(x) inverttheta(x,4), allTracks, 'UniformOutput',false);

figure
hold on
cellfun(@(x) plot(x(:,2),x(:,3)),allTracks)
% NEED TO ADD SAVING OPTION
if options.save
    % Need to save tracks and options
    %options.nFrames = nFrames;
    save([outputPath 'allBacteriaTracks.mat'], 'allTracks');
    save([outputPath 'bacteriaTrackingOptions.mat'], 'options');
