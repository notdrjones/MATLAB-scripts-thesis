...
if nargin<3
        processoptions.cropVideos = false;
        processoptions.track3D = false;
        prepOptions.binning=2;
        prepOptions.bgSubtract=true;
    end

    if ~isfield(processoptions, 'cropVideos'), processoptions.cropVideos = false; end
    if ~isfield(processoptions, 'track3D'), processoptions.track3D = false; end
    
    if ~isfield(prepOptions, 'binning'), prepOptions.binning = 2; end
    if ~isfield(prepOptions, 'bgSubtract'), prepOptions.bgSubtract = false; end


    outputpath = [folderpath '\trainingSetFlatField'];
    ...
    for i=1:length(folders)/4
        ...
        L = loadtiff([currentFolder '\Lflatfield.tif']);
