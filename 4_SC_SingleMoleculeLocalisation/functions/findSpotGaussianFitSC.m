function [x_centre, y_centre, varargout] = findSpotGaussianFitSC(image_data,x_estimate,y_estimate,options)
% TO CHANGE, COMPARED TO ORIGINAL VERSION
% varargout, so one doesn't have to set up all those variables to start
% with
% options structure instead of a bunch of variables.
%
%
...
%just made it a bit more legible and tidied up inputs and outputs.
%
...
if nargin<4 % you need at least image, x, y
    options.subarray_halfwidth = 8; % px
    options.inner_radius = 5; % px
    options.sigma_gauss = 2; % px
    options.error_set = 0.05; % px
    options.clip_override =1 ;
    options.show_output = false;
end

if ~isfield(options, 'subarray_halfwidth'), options.subarray_halfwidth = 8; end
if ~isfield(options, 'inner_radius'), options.inner_radius = 5;end
if ~isfield(options, 'sigma_gauss'), options.sigma_gauss = 2;end
if ~isfield(options, 'error_set'), options.error_set = 0.05;end
if ~isfield(options, 'clip_override'), options.clip_override = 1;end
if ~isfield(options, 'show_output'), options.show_output = false;end


%-- set all values
subarray_halfwidth = options.subarray_halfwidth;
inner_radius = options.inner_radius;
sigma_gauss = options.sigma_gauss;
error_set = options.error_set;
clip_override = options.clip_override;
...
% set values into varargout
varargout_cell = {Ibg_avg, Isp, Idata, bg_noise_std,  mask_pixels, clipping_flag, noConvergenceFlag};
for i=1:nargout
    varargout{i} = varargout_cell{i};
end
